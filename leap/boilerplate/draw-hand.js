...
//
// オリジナルは https://github.com/jaanga/gestification/tree/gh-pages/cookbook/boilerplate
//
...
Leap.loop( function( frame ) {
	//
	// leap-0.6.0-beta2.js で動作する
	// leap-0.6.0.js で動かすと指の表示が異なる
	//
	...
	for (var i = 0; i < iLen; i++) {
		for ( var j = 0; j < 3; j++) {
			phalanx = phalanges[ 3 * i + j]; // 指の骨を参照

			point = frame.pointables[i].positions[j];
			phalanx.position.set( point[0], point[1], point[2] ); // 指の骨の位置をセット

			point = frame.pointables[i].positions[ j + 1 ]; // ひとつ先の骨の位置
