                            ...
                            int *key, int plainLength)
{
    ...
    while (currentPlain != NULL)
    {
        ...
        do
        {
            ...
            x1 = (int)((((unsigned long)x1 * key[ik++]) % 0x10001L) & 0xffff);
            x2 = ((x2 + key[ik++]) & 0xffff);
            x3 = ((x3 + key[ik++]) & 0xffff);
            // x4 = (int)(((x4 * key[ik++]) % 0x10001L) & 0xffff);
            x4 = (int)((((unsigned long)x4 * key[ik++]) % 0x10001L) & 0xffff);

            t2 = (x1 ^ x3);
            // t2 = (int)(((t2 * key[ik++]) % 0x10001L) & 0xffff);
            t2 = (int)((((unsigned long)t2 * key[ik++]) % 0x10001L) & 0xffff);

            t1 = ((t2 + (x2 ^ x4)) & 0xffff);
            // t1 = (int)(((t1 * key[ik++]) % 0x10001L) & 0xffff);
            t1 = (int)((((unsigned long)t1 * key[ik++]) % 0x10001L) & 0xffff);
            t2 = (t1 + t2 & 0xffff);
                   ...
                   encryptChunk **cryptTailOut)
{
    ...
    while (nread < textLen)
    {
        signed char *textData = (signed char *)malloc(CHUNK_SIZE * sizeof(signed char));
        ...
        newChunk->data = (signed char *)malloc(CHUNK_SIZE * sizeof(signed char));
        newChunk->next = NULL;
...
void cleanup(encryptChunk *textList, encryptChunk *cryptList, int *key, int16_t *userkey)
{
    free(key);
    free(userkey);
