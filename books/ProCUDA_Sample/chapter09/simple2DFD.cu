    ...
    float **g_u2)
{
    ...
    if (ngpus > 1)
    {
        ...
        for (int i = 0; i < ngpus; i++)
        {
            ...
            CHECK(cudaMemcpy(iwave + ioff, g_u2[i] + iskip, gsize * sizeof(float),
...
inline bool isCapableP2P(int ngpus)
{
    ...
    cudaDeviceProp *prop = (cudaDeviceProp *)malloc(ngpus);
    int iCount = 0;
...
int main(int argc, char *argv[])
{
    ...
    float **d_u2, **d_u1;
    d_u2 = (float **)malloc(ngpus * sizeof(float *));
    d_u1 = (float **)malloc(ngpus * sizeof(float *));
    ...
    for (int i = 0; i < ngpus; i++)
    {
        ...
        CHECK(cudaMalloc((void **)&d_u1[i], ibyte * sizeof(float)));
        CHECK(cudaMalloc((void **)&d_u2[i], ibyte * sizeof(float)));

        //CHECK(cudaMemset(d_u1[i], 0, ibyte));
        //CHECK(cudaMemset(d_u2[i], 0, ibyte));
        CHECK(cudaMemset(d_u1[i], 0, ibyte * sizeof(float)));
        CHECK(cudaMemset(d_u2[i], 0, ibyte * sizeof(float)));
    ...
    // stream definition
    //cudaStream_t stream_halo[ngpus], stream_internal[ngpus];
    cudaStream_t *stream_halo = (cudaStream_t *)malloc(ngpus * sizeof(cudaStream_t));
    cudaStream_t *stream_internal = (cudaStream_t *)malloc(ngpus * sizeof(cudaStream_t));

    for (int i = 0; i < ngpus; i++)
    {
        CHECK(cudaSetDevice(i));
        CHECK(cudaStreamCreate(&stream_halo[i]));
        CHECK(cudaStreamCreate(&stream_internal[i]));
    }

    // calculate index for computation
    //int haloStart[ngpus], bodyStart[ngpus], haloEnd[ngpus], bodyEnd[ngpus];
    int *haloStart = (int *)malloc(ngpus * sizeof(int));
    int *bodyStart = (int *)malloc(ngpus * sizeof(int));
    int *haloEnd   = (int *)malloc(ngpus * sizeof(int));
    int *bodyEnd   = (int *)malloc(ngpus * sizeof(int));

    calcIndex(haloStart, haloEnd, bodyStart, bodyEnd, ngpus, iny);

    //int src_skip[ngpus], dst_skip[ngpus];
    int *src_skip = (int *)malloc(ngpus * sizeof(int));
    int *dst_skip = (int *)malloc(ngpus * sizeof(int));
    ...
    for (int istep = 0; istep < nsteps; istep++)
    {
        ...
        for (int i = 0; i < ngpus; i++)
        {
            ...
            // compute halo
            kernel_2dfd<<<grid, block, 0, stream_halo[i]>>>(d_u1[i], d_u2[i], nx,
                                                            haloStart[i], haloEnd[i]);

            // compute internal
            kernel_2dfd<<<grid, block, 0, stream_internal[i]>>>(d_u1[i], d_u2[i], nx,
                                                            bodyStart[i], bodyEnd[i]);
        }
    ...
    for (int i = 0; i < ngpus; i++)
    {
        ...
        CHECK (cudaStreamDestroy(stream_internal[i]));
