...
int main(int argc, char *argv[])
{
    ...
    printf("node=%d(%s): my_other_proc = %d and using GPU=%d\n", rank,
    ...
    for(int size = 1024; size <= MAX_MSG_SIZE; size = size * 4)
    {
        ...
        if(rank == 0)
        {
            ...
            for(int i = 0; i < loop; i++)
            {
                ...
                MPI_Irecv(h_rcv, size, MPI_CHAR, other_proc, 10,
        ...
        else
        {
            for(int i = 0; i < loop; i++)
            {
                ...
                MPI_Irecv(h_rcv, size, MPI_CHAR, other_proc, 100,
                          MPI_COMM_WORLD, &recv_request);
                MPI_Isend(h_src, size, MPI_CHAR, other_proc, 10,
                          MPI_COMM_WORLD, &send_request);
        ...
        if(rank == 0)
        {
            ...
            printf("%6d %s %10.2f us %10.2f MB/sec\n",
