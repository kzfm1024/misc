...
int main(int argc, char *argv[])
{
    ...
    printf("node=%d(%s): my_other_proc = %d and using GPU=%d loop %d\n",
           rank, processor, other_proc, igpu, loop);
    ...
    for (int size = 1; size <= MAX_MSG_SIZE; size *= 2)
    {
        ...
        if(rank == 0)
        {
            ...
            for(int i = 0; i < loop; i++)
            {
                ...
                MPI_Irecv(d_rcv, size, MPI_CHAR, other_proc, 10,
        ...
        else
        {
            for(int i = 0; i < loop; i++)
            {
                MPI_Isend(d_src, size, MPI_CHAR, other_proc, 10,
                          MPI_COMM_WORLD, &send_request);
