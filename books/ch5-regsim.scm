...
;;
;; assemble, extract-labels を多値を使用して書き直す
;;
(define (assemble controller-text machine)
  (receive (insts labels) (extract-labels controller-text)
    ;(print insts) ; DEBUG
    ;(print labels) ; DEBUG
    (update-insts! insts labels machine)
    insts))
...
(define (extract-labels text)
  (if (null? text)
      (values '() '())
      (receive (insts labels) (extract-labels (cdr text))
        (let ((next-inst (car text)))
          (if (symbol? next-inst)
              (values insts
                      (cons (make-label-entry next-inst insts) labels))
              (values (cons (make-instruction next-inst) insts)
                      labels))))))
