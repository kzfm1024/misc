...
//光源
float lightPos[] =  {0.0, 8.0, -5.0, 1.0};//光源位置
float lightPos0[] = {0.0, 8.0, -5.0, 1.0};//光源位置(初期値)
//影のマテリアル
...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("異方性反射モデル(SL_Anisotropic)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "anisotropic.vert", "anisotropic.frag");
...
void display(void)
{
	...
	if(flagHelp)
  {
		...
		printf("[F1]キー：粗さ係数roughX調整 \n");
		printf("[F2]キー：粗さ係数roughY調整 \n");
		printf("[F3]キー：接線ベクトル切り替えchangeTangent \n");
...
//	drawAnisoCube(1.0);
	drawAnisoCylinder(0.5, 0.3, 0.5, 30, 5, 10);
