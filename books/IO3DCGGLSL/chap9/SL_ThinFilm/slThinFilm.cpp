...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("薄膜(SL_ThinFilm)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "film.vert", "film.frag");
...
void display(void)
{
	...
	if(flagHelp)
  {
		...
		printf("[F1]キー：膜厚filmSpace調整 \n");
		printf("[F2]キー：薄膜の屈折率refract調整 \n");
		printf("[F3]キー：混合係数blendFactor調整 \n");
