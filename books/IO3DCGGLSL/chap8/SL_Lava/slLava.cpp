...
void makeLavaImage(Bitmap *bm);
void setTerrainTexture(int n);
void setLavaTexture(int n);
void drawParticle(float dt);
void drawLava(float dt);
		...
		for(i = 0; i < nx; i++)
    {
      ii = i * (int)(pWidth / (float)nx);
			...
			else//32bit color
			{
				...
				//黒を透明に
				texImage[j][i][3] = texImage[j][i][0] ;//255;
			}
    }
  }
}
...
void setLavaTexture(int n)
{
 ...
 //テクスチャの割り当て 
  ...
  float atten[] = {1.0, 0.0, 0.001};
//  float atten[] = {1.0, 0.0, 1.0};
  glPointParameterfv =
...
void display(void)
{
	...
	if(flagHelp)
  {
		...
		printf("[F5]キー：x方向の風力変更 \n");
		printf("[F6]キー：光源の明るさ変更 \n");
		flagHelp = false;
  }
  //終了
  glutSwapBuffers();
...
void drawParticle(float dt)
{
  ...
  glDepthMask(GL_FALSE); //ﾃﾞﾌﾟｽﾊﾞｯﾌｧを書き込み禁止
	...
	for(i = 0; i < num0; i++) 
	{
		p[countP + i].create(elapseTime2);//粒子の生成
	}
	countP += num0;
...
void drawLava(float dt)
{
  ...
  glEnable(GL_ALPHA_TEST);
	...
	for(i = 0; i < num0L; i++) 
	{
		q[countL + i].create(elapseTime2);//溶岩粒子の生成
	}
	countL += num0L;
  if(countL + num0 > NUM_LAVA) countL = 0;

  for(i = 0; i < NUM_LAVA; i++) 
  {
		//粒子の位置に相当するTerrainの高さを求める
		if(q[i].vPosition.x <= - sizeX * sc / 2.0 || q[i].vPosition.x >= sizeX * sc / 2.0) continue;
		if(q[i].vPosition.z <= - sizeY * sc / 2.0 || q[i].vPosition.z >= sizeY * sc / 2.0) continue;
		q[i].heightT = getHeightTerrain(q[i].vPosition.x, q[i].vPosition.z);//terrain.hにある
		q[i].show(elapseTime2);
...
void setLight()
{
  float lightAmbient0[] = {light, light, light, 1.0f}; //環境光
  float lightDiffuse0[] = {light, light, light, 1.0f}; //拡散光
  float lightSpecular0[] = {0.2, 0.2f, 0.2f, 1.0f}; //鏡面光
