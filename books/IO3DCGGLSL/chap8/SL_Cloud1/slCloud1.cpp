...
View view = { 
	0.0, 0.0, 0.0,//pos(仮設定）
  0.0, 5.0, 0.0,//cnt 
	10.0, 5.0, 0.0,//dist, theta, phi
	80.0, 1.0, 1000.0//fovY,nearZ, farZ
};
View view0 = view;
...
void display(void)
{
	...
	if(flagHelp)
  {
		...
		printf("[F1]キー：x方向の風力変更 \n");
		printf("[F2]キー：一度に放出する粒子数num0変更 \n");
		flagHelp = false;
  }
  //終了
  glutSwapBuffers();
...
void special(int key, int x, int y)
{
  ...
  if(key == GLUT_KEY_F1) 
  {
		if(glutGetModifiers() == GLUT_ACTIVE_SHIFT) accelX -= 0.1; 
		else accelX += 0.1; 
		printf("accelX = %f \n", accelX);
  }	
  //num0
...
void motion(int x, int y)
{
  ...
  view.theta += 0.1 * (float)(y - yStart) ;//crane
