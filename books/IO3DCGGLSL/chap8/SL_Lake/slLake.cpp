...
//光源
float lightPos[] =  {0.0, 15.0, -5.0, 1.0};//光源位置
float lightPos0[] = {10.0, 15.0, 0.0, 1.0};//光源位置(初期値)
...
View view = { 
	0.0, 0.0, 0.0,//pos(仮設定）
  0.0, 1.0, 0.0,//cnt 
	8.0, 30.0, 0.0,//dist, theta, phi
	70.0, 1.0, 100.0//fovY,nearZ, farZ
};
View view0 = view;
...
//  glGenTextures(1, &texName);//テクスチャオブジェクトの名前付け
  ...
  loadBitmap(bm0, "../../bmp/altgrad2.bmp");
...
void special(int key, int x, int y)
{
  ...
  if(key == GLUT_KEY_F4) 
  {
	  if(glutGetModifiers() == GLUT_ACTIVE_SHIFT) alphaWave -= 0.1;
	  else alphaWave += 0.1;
		printf("alphaWave = %f \n", alphaWave);
	  makeTerrainData();
  }  else
