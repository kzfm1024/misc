...
CmyTexPet::CmyTexPet()
{
	...
	static float diffuse0[] = {0.9f, 0.6f, 0.4f, 1.0f};
  static float ambient0[] = {0.5f, 0.3f, 0.2f, 1.0f};
...
//------------------------------------------------------------------------------
void CmyTexPet::drawTrunk(void)
{
	//胴体
	glPushMatrix();
		glScalef(trunk.x, trunk.y, trunk.z);
    glRotatef(90.0, 1.0, 0.0, 0.0);//90度x軸回転
		drawTexSuper(0.5, 16, 8, 0.5, 0.5);
	glPopMatrix();
}
//-----------------------------------------------------------------------------
void CmyTexPet::drawHead()
{
...
//----------------------------------------------------------------------------
void CmyTexPet::drawLeg1(int jNo)
{
  //脚付け根の回転
  ...
  glPushMatrix();
    ...
    glTranslatef(0.0, -0.5, 0.0);//原点の位置へ下げる
		drawTexSuper(0.5, 8, 8, 0.5, 0.8);
  glPopMatrix();
}
//----------------------------------------------------------------------------
void CmyTexPet::drawLeg2(int jNo)
{
  //下脚の位置へ平行移動
...
void CmyTexPet::initKeyAnimation()
{
	short k, j;
	//現在の位置姿勢（最初の姿勢で初期化)
...
//----------------------------------------------------------------
//与えられた距離歩く
void CmyTexPet::walk(float dist,  float step, float time)//
...
//---------------------------------------------------------------------
void CmyTexPet::walkRight(float step, float time)
{
...
//---------------------------------------------------------------------
void CmyTexPet::walkLeft(float step, float time)
{
...
void CmyTexPet::turn(float angle, float time)
{
	numKeyFrame = 2;
	initKeyAnimation();
...
//後ろ脚で立ち上がる
void CmyTexPet::stand(float time)
{
	numKeyFrame = 2;
	initKeyAnimation();
...
//基本姿勢に戻る
void CmyTexPet::initPose(float time)
{
	short j;
...
void CmyTexPet::wait(float time)
{
	numKeyFrame = 2;
	initKeyAnimation();
	actTime[1] = time;
...
void CmyTexPet::swingTail(int n, float time)
{
	short i;
...
void CmyTexPet::upTail(float time)
{
	numKeyFrame = 2;
	initKeyAnimation();
	k_joint[1][9].x += 60.0;
...
void CmyTexPet::downTail(float time)
{
	numKeyFrame = 2;
	initKeyAnimation();
	k_joint[1][9].x -= 60.0;
