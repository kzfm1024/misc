...
void draw0();
void draw1();
void drawGround(float widthX, float widthZ, int nx, int nz);
...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("バンプマッピング2(SL_BumpMap2)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "bumpMap2.vert", "bumpMap2.frag");
  //イベント処理ループに入る
  glutMainLoop();
	//リソースの解放
...
void makeTexture()
{
	...
	loadBitmap(bm2, "../../bmp/stones1.bmp");
	makeTexImage(bm2);
	setColorMap(2);
	makeNormalMap(bm2);
	setNormalMap(2);
...
void draw0(void)
{
  ...
  if(!flagShadow)
  {
		...
		//drawBumpTorus(0.25, 0.45, 20, 20, 16, 4);
  }
  else
  {//影表示のときはTextureを付けない
	  drawCube(1.0);
...
void drawGround(float widthX, float widthZ, int nx, int nz)
{
  ...
  for (j = 0; j < nz; j++) 
	{
    z1 = wZ * (float)j; z2 = z1 + wZ;
    for (i = 0; i < nx; i++) {
      x1 = wX * (float)i; x2 = x1 + wX;

			glBegin(GL_QUADS);
				//頂点座標
				glTexCoord2f(0.0, 1.0); glVertex3f(x1, 0.0, z1);
				glTexCoord2f(0.0, 0.0); glVertex3f(x1, 0.0, z2);
				glTexCoord2f(1.0, 0.0); glVertex3f(x2, 0.0, z2);
				glTexCoord2f(1.0, 1.0); glVertex3f(x2, 0.0, z1);
			glEnd();
    }
  }
  glPopMatrix();
