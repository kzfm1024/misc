...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("変位マッピング2(SL_Displacement2)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "displacement2.vert", "displacement2.frag");
  //イベント処理ループに入る
  glutMainLoop();
...
void display(void)
{
  ...
  //fragment shaderのユニフォーム変数のインデックスを取得
	...
	GLint biasLoc = glGetUniformLocation(shaderProg, "bias");
...
void special(int key, int x, int y)
{
  ...
  if(key == GLUT_KEY_F4) 
  {
		if(glutGetModifiers() == 0) bias += 0.02;
		if(glutGetModifiers() == GLUT_ACTIVE_SHIFT) bias -= 0.02;
		printf("bias = %f \n", bias);
  }	
