...
キューブマッピングによる屈折環境ﾊﾞﾝﾌﾟﾏｯﾋﾟﾝｸﾞ
framebuffer objectによるoff-screen rendering
OpenGL+GLSL
*/
#include <stdio.h>
#include <windows.h>
...
int fps = 0;
int actionMode = 0;
float transparency0 = 0.8;//透明度
float nRatio0 = 2.0;//比屈折率
...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("屈折ﾊﾞﾝﾌﾟﾏｯﾌﾟ(SL_RefractBump)");
  //ウィンドウのサイズ変更
  ...
  init();
	initFramebuffer();
  initGlsl(&shaderProg, "refractBump.vert", "refractBump.frag");
  //イベント処理ループに入る
  glutMainLoop();
...
void init(void)
{
 //背景色
  ...
  setCubeMap();//テクスチャの設定
  //pet robot アニメーション
  pet.unitTime = 0.05;//１フレーム当たりの時間（最初は仮の値）
  //フレームデータを作成
  pet.numFrame0 = 0;
	pet.vPos.x =  1.0;
	pet.vPos.z = -1.0;
...
void display(void)
{
  ...
  glBindTexture(GL_TEXTURE_CUBE_MAP, texName[0]);
  GLint samplerLoc = glGetUniformLocation(shaderProg, "cubeMap");
