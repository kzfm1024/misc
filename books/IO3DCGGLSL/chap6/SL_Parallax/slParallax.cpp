...
float fShift = 0.05;//視線ベクトルシフト量調整ファクタ
...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("視差マッピン(SL_Parallax)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "parallax.vert", "parallax.frag");
  //イベント処理ループに入る
  glutMainLoop();
	//リソースの解放
...
//  loadBitmap(bm0, "../../bmp256/char256-0.bmp");
  loadBitmap(bm0, "../../bmp256/disk4-256.bmp");
  makeNormalMap(bm0);
  setNormalMap(0);
  //obj1用
  Bitmap* bm1 = new Bitmap();
//  loadBitmap(bm1, "../../bmp256/char256-0.bmp");//disk4-256.bmp");
...
void display(void)
{
  ...
  glUseProgram(shaderProg);
	...
	GLint shiftLoc = glGetUniformLocation(shaderProg, "fShift");
	glUniform1f(shiftLoc, fShift);
...
void special(int key, int x, int y)
{
  ...
  if(key == GLUT_KEY_F3) 
  {
		if(glutGetModifiers() == 0) fShift += 0.01;
		if(glutGetModifiers() == GLUT_ACTIVE_SHIFT) fShift -= 0.01;
