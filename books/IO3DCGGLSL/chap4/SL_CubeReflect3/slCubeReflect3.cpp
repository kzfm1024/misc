...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("反射マッピング3(SL_CubeReflect3)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "cubeReflect3.vert", "cubeReflect3.frag");
...
void display(void)
{
  ...
  GLuint refLoc = glGetUniformLocation(shaderProg, "reflectivity");
  glUniform1f(refLoc, reflectivity0);
  float m[16] ;
  glGetFloatv(GL_MODELVIEW_MATRIX, m);
  GLuint ViewTransLoc = glGetUniformLocation(shaderProg, "ViewTranspose");
