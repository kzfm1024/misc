...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("ｸﾞｰﾛｰｼｪｰﾃﾞｨﾝｸﾞ(SL_Gouraud)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "Gouraud.vert");
...
void display(void)
{
	...
	if(flagHelp)
  {
		...
		printf("'f'でｽﾑｰｽﾞとﾌﾗｯﾄｼｪｰﾃﾞｨﾝｸﾞの切り替え \n");
		printf("マウス操作で視点変更可 \n");
...
void keyboard(unsigned char key, int x, int y)
{
  ...
  case 'w':
	  ...
	  break;
  case 'f':
	  if(flagFlat) flagFlat = false;
	  else if(!flagFlat) flagFlat = true;
	  break;
  case 'h':
