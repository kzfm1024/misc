...
double result;
  if (k == 0) {
    result = CmR(ns2[N - 1].g[x][y], ns2[0].g[x][y], ns2[1].g[x][y], ns2[2].g[x][y], t);
  }
  else if (k < N - 2) {
    result = CmR(ns2[k - 1].g[x][y], ns2[k].g[x][y], ns2[k + 1].g[x][y], ns2[k + 2].g[x][y], t);
  }
  else //(k = N - 2) 
  {
    result = CmR(ns2[N - 3].g[x][y], ns2[N - 2].g[x][y], ns2[N - 1].g[x][y], ns2[0].g[x][y], t);
  }
	if(result < -1.0)
	{
		printf("result = %f \n", result);
