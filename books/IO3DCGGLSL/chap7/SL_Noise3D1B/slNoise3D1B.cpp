...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("雑音3D1B(SL_Noise3D1B)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
	glewInit();
  init();
  initGlsl(&shaderProg, "noise3D1B.vert", "noise3D1B.frag");
  //イベント処理ループに入る
  glutMainLoop();
	//リソースの解放
		...
		for (j = 0; j < NOISE_SIZE; j++) 
		{
			for (i = 0; i < NOISE_SIZE; i++) 
			{
				if(texPattern == PERLIN)
					texImage[k][j][i] =
						(GLubyte)((1.0 + noise3.perlin(i, j, k, 5)) * 127.0);
				else // TURBULENCE
					texImage[k][j][i] =
						(GLubyte)(noise3.turbulence(i, j, k, 5) * 255.0);
			}
		}
  } 
}
...
void display(void)
{
 //時間計測
  ...
  glUseProgram(shaderProg);
	...
	GLint thresholdLoc = glGetUniformLocation(shaderProg, "threshold");
...
void special(int key, int x, int y)
{
	...
	if(key == GLUT_KEY_F1) 
  {
	...
	if(key == GLUT_KEY_F3) 
  {
	  if(glutGetModifiers() == 0) threshold += 0.05;
	  if(glutGetModifiers() == GLUT_ACTIVE_SHIFT) threshold -= 0.05;
