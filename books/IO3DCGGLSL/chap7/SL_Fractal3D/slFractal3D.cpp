...
void makeTexImage3D(int texPattern);
...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("フラクタル3D(SL_Fractal3D)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "fractal3D.vert", "fractal3D.frag");
  //イベント処理ループに入る
  glutMainLoop();
	//リソースの解放
...
void init(void)
{
 //背景色
  ...
  printf("フラクタル集合による3Dテクスチャを計算しています \n");
  //３DテクスチャーのImageをあらかじめ作成
  glGenTextures(2, texName);
  makeTexImage3D(MANDEL1);
//  makeTexImage3D(JULIA1);
  setTexture3D(0);
  makeTexImage3D(MANDEL2);
//  makeTexImage3D(JULIA2);
  setTexture3D(1);
}
...
{
  int i, j, k;
  GLubyte image[TEX_WIDTH * TEX_HEIGHT][3];
  CFractal fractal;
  fractal.widthX = TEX_WIDTH / 2;
  fractal.widthY = TEX_HEIGHT / 2;

  for (k = 0; k < TEX_DEPTH; k++) 
  {
		if(texPattern == MANDEL1)
		{
			fractal.numRepeat = 30;
			fractal.posX = -0.1;
			fractal.posY = 0.9;
			fractal.scale = 0.02;
			fractal.initX = 0.0;
			fractal.initY = 0.002*(double)k;
			fractal.mandel(image);
		}
		else if(texPattern == MANDEL2)
		{
			fractal.numRepeat = 100;
			fractal.posX = -1.214;
			fractal.posY = -0.328;
			fractal.scale = 0.1 + 0.0001*(double)k;
			fractal.initX = 0.0;
			fractal.initY = 0.0;
			fractal.mandel(image);
		}

		if(texPattern == JULIA1)
		{
			fractal.numRepeat = 300;
			fractal.posX = 0.0;
			fractal.posY = 0.0;
			fractal.scale = 0.8;
			fractal.constA = -0.743+ 0.00002*(double)k;
			fractal.constB = 0.113;
			fractal.julia(image);
		}
		else if(texPattern == JULIA2)
		{
			fractal.numRepeat = 200;
			fractal.posX = -0.245;
			fractal.posY = 0.1;
			fractal.scale = 0.1 + 0.001*(double)k;
			fractal.constA = -0.745;
			fractal.constB = 0.113;
			fractal.julia(image);
		}
		...
		for (j = 0; j < TEX_HEIGHT; j++) 
		{
			...
			{
				texImage[k][j][i][0] = image[j*TEX_WIDTH+i][0];
				texImage[k][j][i][1] = image[j*TEX_WIDTH+i][1];
				texImage[k][j][i][2] = image[j*TEX_WIDTH+i][2];
			}//i
		}//j
  }//k
}
