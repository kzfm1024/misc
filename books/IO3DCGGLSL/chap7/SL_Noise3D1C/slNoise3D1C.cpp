...
//ﾃｸｽﾁｬ用
GLubyte texImage[NOISE_SIZE][NOISE_SIZE][NOISE_SIZE];
GLuint texName[2];
//シェーダプログラム名
GLuint shaderProg;
...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("雑音3D1C(SL_Noise3D1C)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
	glewInit();
  init();
  initGlsl(&shaderProg, "noise3D1C.vert", "noise3D1C.frag");
  //イベント処理ループに入る
  glutMainLoop();
	//リソースの解放
...
void init(void)
{
 //背景色
  ...
  makeTexImage3D(PERLIN);
  setTexture3D(0);
  makeTexImage3D(TURBULENCE);
  setTexture3D(1);
}
...
void setTexture3D(int n)
{
  ...
  glTexImage3D(GL_TEXTURE_3D,0,GL_LUMINANCE,NOISE_SIZE,NOISE_SIZE,NOISE_SIZE,0,GL_LUMINANCE,GL_UNSIGNED_BYTE,texImage);
...
void makeTexImage3D(int texPattern)
{
  ...
  for (k = 0; k < NOISE_SIZE; k++) 
  {
		for (j = 0; j < NOISE_SIZE; j++) 
		{
			...
			{
				if(texPattern == PERLIN)
					texImage[k][j][i] = (GLubyte)((1.0 + noise3.perlin(i, j, k, 5)) * 127.0);
				else // TURBULENCE
					texImage[k][j][i] = (GLubyte)(noise3.turbulence(i, j, k, 5) * 255.0);
			}
		}
  } 
}
