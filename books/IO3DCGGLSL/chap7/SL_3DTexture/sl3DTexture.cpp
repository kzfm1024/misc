...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("3Dテクスチャ(SL_3DTexture)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
	glewInit();
  init();
  initGlsl(&shaderProg, "3DTexture.vert", "3DTexture.frag");
  //イベント処理ループに入る
  glutMainLoop();
  ...
  for (k = 0; k < TEX_DEPTH; ++k) 
  {
		...
		for (j = 0; j < TEX_HEIGHT; ++j) 
		{
			...
			for (i = 0; i < TEX_WIDTH; ++i) 
			{
				...
				texImage[k][j][i][1] = 127 - (GLbyte)(gray * 127.0);
  ...
  for (k = 0; k < TEX_DEPTH; ++k) 
  {
		for (j = 0; j < TEX_HEIGHT; ++j) 
		{
			for (i = 0; i < TEX_WIDTH; ++i) 
			{
				...
				texImage[k][j][i][0] = texImage[k][j][i][1] = texImage[k][j][i][2] =150 + (GLubyte)rr;
