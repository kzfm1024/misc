...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("集光模様2(SL_Caustics2)");
  //ウィンドウのサイズ変更
...
void setTextureCaustics()
{//集光模様用
  glBindTexture(GL_TEXTURE_2D, texName[0]);//波テクスチャをバインドする
...
void setCubeMap()
{//キューブマップ用
  glBindTexture(GL_TEXTURE_CUBE_MAP, texName[1]);//環境マップをバインドする
  for(int i = 0; i < 6; i++)
	glTexImage2D(target[i].name,0,GL_RGBA,TEX_WIDTH2,TEX_HEIGHT2,0,GL_RGBA,GL_UNSIGNED_BYTE,0);
