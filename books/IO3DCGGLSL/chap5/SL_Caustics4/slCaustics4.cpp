...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("集光模様4(SL_Caustics4)");
  //ウィンドウのサイズ変更
...
void display(void)
{
  ...
  for (int i = 0; i < 6; ++i)
    ...
    glViewport(0, 0, TEX_WIDTH2, TEX_HEIGHT2);
		...
		glPushMatrix();
		//視点から見えるものをレンダリング 
		gluLookAt(view.pos[0], view.pos[1], view.pos[2], view.pos[0]+target[i].cx, view.pos[1]+target[i].cy, view.pos[2]+target[i].cz,
  ...
  glUniform1i(texLoc2, 0);//GL_TEXTURE1を適用
