...
//光源
float lightPos[] =  {3.0, 6.0, 1.0, 1.0};//光源位置
float lightPos0[] = {3.0, 6.0, 1.0, 1.0};//光源位置(初期値)
...
int TEX_WIDTH = 256;
int TEX_HEIGHT = 256;
float fov = 90.0;//ｼｬﾄﾞｳﾏｯﾋﾟﾝｸﾞの視野角
float farZ = 50.0;//ｼｬﾄﾞｳﾏｯﾋﾟﾝｸﾞの後方クリップ面
float dis0 = 0.05;//サンプリング変位
...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("シャドウマッピング2(SL_ShadowMap2)");
  //ウィンドウのサイズ変更
...
void display(void)
{
 ...
 //描画
  ...
  glUniform1i(texLoc, 0);//GL_TEXTURE0を適用
	GLint disLoc = glGetUniformLocation(shaderProg, "dis");
	glUniform1f(disLoc, dis0);
	draw0(); 
	draw1();
  drawFloor0(10.0, 10.0, 10, 10);
...
void draw1(void)
{
  ...
  glScalef(scale[1][0], scale[1][1], scale[1][2]);
//	glutSolidCube(1.0);
	drawCylinder2(1.0, 0.7, 0.5, 0.2, 20, 5);
  glPopMatrix();
}
...
void special(int key, int x, int y)
{
  ...
  if(key == GLUT_KEY_F3) 
  {
		if(glutGetModifiers() == 0) dis0 += 0.01;
		if(glutGetModifiers() == GLUT_ACTIVE_SHIFT) dis0 -= 0.01;
		if(dis0 < 0.0) dis0 = 0.0;
	  printf("dis = %f \n", dis0);
  }
