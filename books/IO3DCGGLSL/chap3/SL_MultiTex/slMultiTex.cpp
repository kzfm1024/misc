...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("マルチテクスチャ(SL_MultiTex)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "MultiTex.vert", "MultiTex.frag");
  //イベント処理ループに入る
  glutMainLoop();
	//リソースの解放
...
void init(void)
{
 //背景色
  ...
  loadBitmap(bm1, "../../bmp/matumae1.bmp");
  makeTexImage(bm1);
  setTexture(1);
}
void makeCheckImage(int numCheck)
  ...
  for(j = 0; j < TEX_HEIGHT; j++)
	{
		...
		for(i = 0; i < TEX_WIDTH; i++)
		{
			...
			texImage[j][i][3] = 255; //(unsigned char)255;
		}	
	}
}
...
void display(void)
{
  ...
  glUseProgram(shaderProg);
	...
	GLint mixLoc = glGetUniformLocation(shaderProg, "mixFactor");
	glUniform1f(mixLoc, mixFactor);
	//テクスチャユニットをシェーダ側のサンプラに関連付ける
  GLint samplerLoc0 = glGetUniformLocation(shaderProg, "sampler0");
  GLint samplerLoc1 = glGetUniformLocation(shaderProg, "sampler1");
...
//		printf("描画時間 = %4.4f[s] \n", drawTime);
	...
	if(flagHelp)
  {
		...
		printf("[F1]キー：テクスチャ混合率mixFactorの調整 \n");
		flagHelp = false;
  }
  //終了
  glutSwapBuffers();
...
void draw0(void)
{
  ...
  if(!flagShadow)
  {
		...
		//drawTexTorus(0.2, 0.5, 20, 20, 1, 1);
  }
  else
  {//影表示のときはTextureを付けない
		drawCube(1.0);
...
void special(int key, int x, int y)
{
  ...
  else if(sKey == LIGHT)
  {
	...
	if(key == GLUT_KEY_F1) 
	{
		...
		if(glutGetModifiers() == 0) mixFactor += 0.1;
		if(glutGetModifiers() == GLUT_ACTIVE_SHIFT) mixFactor -= 0.1;
