...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("カラーマッピング(SL_ColorMap)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "colorMap.vert", "colorMap.frag");
  //イベント処理ループに入る
  glutMainLoop();
	//リソースの解放
...
void init(void)
{
 //背景色
  ...
  loadBitmap(bm1, "../../bmp/disk5-8-1.bmp");
  makeTexImage(bm1);
  setTexture(1);
}
...
void display(void)
{
  ...
  //fragment shaderのユニフォーム変数samlerのインデックスを取得
...
void draw0(void)
{
  ...
  if(!flagShadow)
  {
		drawTexTorus(0.2, 0.5, 20, 20, 2, 1 );
  }
  else
  {//影表示のときはTextureを付けない
