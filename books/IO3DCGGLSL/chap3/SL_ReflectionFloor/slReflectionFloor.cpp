...
void draw0R();
void draw1R();
void drawFloorR(float widthX, float widthZ, int nx, int nz);
...
float lightPosR[] = {0.0,-5.0, 0.0, 1.0};//光源位置(反射用）
//影のマテリアル
...
int main(int argc, char** argv)
{
  ...
  //ウィンドウ作成
  glutCreateWindow("床面反射(SL_ReflectionFloor)");
  //ウィンドウのサイズ変更
  ...
  //初期設定
  init();
  initGlsl(&shaderProg, "reflection.vert", "reflection.frag");
  //イベント処理ループに入る
  glutMainLoop();
	//リソースの解放
...
void init(void)
{
 //背景色
  ...
  loadBitmap(bm0, "../../bmp256/tiger1.bmp");
  makeTexImage(bm0);
  setTexture(0);
	//objNo=1
  Bitmap* bm1 = new Bitmap();
  loadBitmap(bm1, "../../bmp/check1.bmp");
  makeTexImage(bm1);
  setTexture(1);
}
...
void display(void)
{
  ...
  glMatrixMode(GL_MODELVIEW);//マトリクスモードを元に戻す
	//鏡像オブジェクトを描く
  glUniform1i(texLoc, 0);//GL_TEXTURE0を適用
  draw0R(); 
  glUniform1i(texLoc, 1);//GL_TEXTURE1を適用
...
void draw0(void)
{
  ...
  float specular[]= { 0.5, 0.5, 0.5, 1.0};
	...
	if(flagShadow) 
  {//shadowDiffuse[3] = shadowDiffuse0[3] * alpha;
  ...
  if(!flagShadow)
  {
		drawTexCube2(1.0, 1, 1);
		//drawTexTorus(0.2, 0.5, 20, 20 );
  }
  else
  {//影表示のときはTextureを付けない
		drawCube(1.0);
	  //drawTorus(0.2, 0.5, 10, 10);
  }
...
void draw1(void)
{
  float ambient[] = { 0.4, 0.1, 0.1, 1.0};
  float diffuse[] = { 0.6, 0.2, 0.2, 1.0};
  float specular[]= { 0.5, 0.5, 0.5, 1.0};
...
void draw1R(void)
{
  ...
  glRotatef(-angle[1][0] - ang, 1.0, 0.0, 0.0);//x軸回転
  glScalef(scale[1][0], scale[1][1], scale[1][2]);
  drawTexSphere2(0.5, 20, 20, 1, 1);  //オブジェクト形状
  glPopMatrix();
}
...
void drawFloorR(float widthX, float widthZ, int nx, int nz)
{
  ...
  float diffuse[][4] = { { 0.8, 0.7, 0.6, alpha}, { 0.3, 0.4, 0.5, alpha} };
...
void keyboard(unsigned char key, int x, int y)
{
  ...
  case 'R'://reset（初期値に戻す)
		for(int j = 0; j < 2; j++)
			...
			{
				pos[j][i] = pos0[j][i]; 
				scale[j][i] = scale0[j][i]; 
				angle[j][i] = angle0[j][i];
			}
		for(int i = 0; i < 3; i++) 
		{
			lightPos[i] = lightPos0[i];
			lightPosR[i] = lightPos0[i];
		}
		lightPosR[1] = -lightPos0[1];
	  view = view0;
	  setCamera();
	  break;
  case 'l':
...
void special(int key, int x, int y)
{
  ...
  else if(sKey == LIGHT)
  {
	  ...
	  default:
		...
		if(lightPos[1] < 0.0) lightPos[1] = 0.0;
	  lightPosR[0] = lightPos[0];
	  lightPosR[1] = -lightPos[1];
	  lightPosR[2] = lightPos[2];
		printf("lightPos.y = %f \n", lightPos[1]);
  }
	if(key == GLUT_KEY_F1) 
	{
