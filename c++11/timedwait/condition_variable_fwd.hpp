...
#define BOOST_THREAD_DEFINES_CONDITION_VARIABLE_NATIVE_HANDLE
    ...
    //
    class condition_variable_steady_np
    {
    private:
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
        pthread_mutex_t internal_mutex;
    ...
    public:
      BOOST_THREAD_NO_COPYABLE(condition_variable_steady_np)
        condition_variable_steady_np()
        {
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
            int const res=pthread_mutex_init(&internal_mutex,NULL);
            if(res)
            {
                boost::throw_exception(thread_resource_error(res, "boost::condition_variable_steady_np::condition_variable_steady_np() constructor failed in pthread_mutex_init"));
            }
#endif
            pthread_condattr_t attr;
            pthread_condattr_init(&attr);
            ...
            if(res2)
            {
                ...
                BOOST_VERIFY(!pthread_mutex_destroy(&internal_mutex));
#endif
                boost::throw_exception(thread_resource_error(res2, "boost::condition_variable_steady_np::condition_variable_steady_np() constructor failed in pthread_cond_init"));
            }
        }
        ~condition_variable_steady_np()
        {
        ...
        template<typename predicate_type>
        void wait(std::unique_lock<std::mutex>& m,predicate_type pred)
        {
            while(!pred()) wait(m);
        }
...
#define BOOST_THREAD_DEFINES_CONDITION_VARIABLE_NATIVE_HANDLE
    ...
    BOOST_THREAD_DECL void notify_all_at_thread_exit(condition_variable_steady_np& cond, std::unique_lock<std::mutex> lk);
    //
